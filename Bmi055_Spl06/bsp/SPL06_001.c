...
uint8_t spl0601_init(void)
{
    ...
    spl0601_get_calib_param();	
	  ...
	  spl06_pressure_rate_config(PRESSURE_RATE_128_TIMES,PRESSURE_RATE_16_TIMES);
		//后台数据采样速率32HZ 过采样率8次//设置传感器上的温度计作为温度采集
	  spl06_temperature_rate_config(PRESSURE_RATE_128_TIMES,TEMPERATURE_RATE_2_TIMES,TEMPERATURE_RATE_TMP_EXT_EXTERNAL);
		//启动后台读取数据
 ...
 * @return     
 **********************************************************************/
float spl0601_get_pressure(float temprature)
{
    float fTsc, fPsc;
    float qua2, qua3;
    float fPCompensate;
...
float user_spl0601_get_presure(float temprature)
{
		...
		return spl0601_get_pressure(temprature);	//气压解算并经过温度补偿后的气压值
}
